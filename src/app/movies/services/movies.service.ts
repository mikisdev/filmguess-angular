...
export class MovieService {
  ...
  private params = new HttpParams().set('api_key', this.apiKey).set('language', 'es-ES').set('include_adult', false);

  private headers = new HttpHeaders().set('Authorization', this.auth);

  constructor(private readonly http: HttpClient) {}

  public getPopular(page: number = 1): Observable<Movie[]> {
  ...
  public getNowPlaying(page: number = 1): Observable<Movie[]> {
    const params = this.params.append('page', page);
  ...
  public getUpcoming(page: number = 1): Observable<Movie[]> {
    const params = this.params.append('page', page);
  ...
  public getTopRated(page: number = 1): Observable<Movie[]> {
    const params = this.params.append('page', page);
  ...
  public getMovieProviders(id: number): Observable<Result> {
    return this.http
      .get<MovieProvidersResponse>(`${this.baseUrl}/movie/${id}/watch/providers`, { headers: this.headers })
      .pipe(
  ...
  public getMovieCast(id: number): Observable<Cast[]> {
    const params = this.params;
